...
### 0.8 Protected Customer Dataブロッカー

`Admin認証だけ確認` は成功し、session scopeにも `write_draft_orders` が含まれる状態になった。

```text
[debug] Admin auth OK
sessionScopes: write_draft_orders,write_metaobject_definitions,write_metaobjects,write_products
```

その後、Draft Order作成で以下のGraphQL errorが返った。
...
- `write_draft_orders` scopeは付与済み。
- ただしDraft OrderはShopifyのProtected Customer Data対象。
- development storeでも、Partner Dashboard側でProtected Customer Data accessと必要フィールドを選択する必要がある。

コード対応:

- この文言を `[protected_customer_data]` として分類する。
- これはコードで解決できる権限ではなく、Partner Dashboardでの人間操作が必要。

人間が次にやること:

1. Partner Dashboardで対象アプリ `b2b-quote-flow-jp` を開く。
2. `API access requests` を開く。
3. `Protected customer data access` を設定する。
4. Draft Order作成で必要なProtected customer dataを選択する。
5. 今回のmutationは `email` を使うため、少なくともEmail fieldを選択する。
6. development storeのみの検証なら、review submitではなく設定保存後に再インストール/権限更新して再テストする。

注意:

- Draft Order作成成功はまだ未確認。
- Protected Customer Data設定後に再度 `Admin認証だけ確認` と `Draft Orderを作成` を実行する。
